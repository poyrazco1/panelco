...
CREATE TABLE IF NOT EXISTS `customers` (
    ...
    `maint_consent_at`         DATETIME     DEFAULT NULL,
    `maint_consent_source`     VARCHAR(60)  DEFAULT NULL,
    `maint_consent_by_user_id` INT UNSIGNED DEFAULT NULL,
    `maint_consent_revoked_at` DATETIME     DEFAULT NULL,
...
/* =========================================================================
 * TEKNİK SERVİS — 6 AYLIK BAKIM HATIRLATMA SİSTEMİ
 * Ayrıntı ve elle kurulum: migrations/2026-07-service-maintenance.sql
 * (Uygulama ayrıca includes/service_maintenance.php::smaint_ensure_schema()
 *  ile çalışma anında da bu tabloları garanti eder.)
 * ====================================================================== */
CREATE TABLE IF NOT EXISTS `service_maintenance_reminders` (
    ...
    KEY `idx_smr_service` (`service_id`), KEY `idx_smr_customer` (`customer_id`),
    KEY `idx_smr_assigned` (`assigned_user_id`), KEY `idx_smr_due` (`maintenance_due_date`),
    KEY `idx_smr_status` (`status`), KEY `idx_smr_stage` (`reminder_stage`),
    KEY `idx_smr_next` (`next_contact_at`), KEY `idx_smr_deleted` (`deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `service_maintenance_communications` (
    ...
    `created_at`     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`), KEY `idx_smc_reminder` (`reminder_id`), KEY `idx_smc_channel` (`channel`),
    KEY `idx_smc_status` (`status`), KEY `idx_smc_created` (`created_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `service_maintenance_status_history` (
    ...
    `created_at`  DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`), KEY `idx_smsh_reminder` (`reminder_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `service_maintenance_postponements` (
    ...
    `postponed_by`   INT UNSIGNED NULL,
    `created_at`     DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`), KEY `idx_smp_reminder` (`reminder_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `service_maintenance_templates` (
    ...
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`), KEY `idx_smt_channel` (`channel`), KEY `idx_smt_active` (`is_active`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS `service_maintenance_cron_logs` (
    ...
    `detail`         TEXT NULL,
    PRIMARY KEY (`id`), KEY `idx_smcl_run` (`run_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
